...
class Analyzer:
    ...
    def stats(self) -> pd.DataFrame:
        ...
        return self._backtest_results.groupby(self._constants.IS_LOSING)[columns].agg(
            func
        )
    ...
    def missed_tp_by(self) -> pd.Series:
        ...
        return (
            losing_trades[self._constants.MAX_PNL]
            / losing_trades[self._constants.ONE_R]
        ).describe()
    ...
    def plot_equity_curve(self, capital: float = 10_000, symbol: str = "") -> None:
        ...
        if not symbol:
            symbol = random.choice(
                self._backtest_results[self._constants.SYMBOL].unique()
            )

        stock = self._backtest_results[
            self._backtest_results[self._constants.SYMBOL] == symbol
        ]
        equity_curve = stock.groupby(self._constants.EXIT_TIME)[
            self._constants.PNL
        ].sum()
    ...
    def _equity_curves_simulation(
        ...
        risk_value_counts = self._backtest_results[
            self._constants.RISK_TO_REWARD
    ...
    def _enrich_results(self) -> None:
        self._backtest_results[self._constants.IS_LOSING] = (
            self._backtest_results[self._constants.PNL] > 0
        )
        self._backtest_results[self._constants.RISK_TO_REWARD] = (
            self._backtest_results[self._constants.EXIT_PRICE]
            - self._backtest_results[self._constants.ENTRY_PRICE]
