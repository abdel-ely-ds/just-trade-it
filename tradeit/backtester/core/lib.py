...
def plot_heatmaps(
    ...
    *,
    ...
    `core.core.Backtest.optimize` when its parameter
    `return_heatmap=True`.
...
http://pandas.pydata.org/pandas-docs/stable/timeseries.html#offset-aliases
    ...
    `core.core.Strategy.data` series. Due to pandas
    ...
    implicit `core.core.Strategy.I` call
    are passed to `func`.
    ...
    def wrap_func(resampled, *args, **kwargs):
        ...
        result = result.reindex(
            index=series.index | resampled.index, method="ffill"
...
def random_ohlc_data(
    example_data: pd.DataFrame, *, frac=1.0, random_state: int = None
) -> pd.DataFrame:
    """
...
class SignalStrategy(Strategy):
    ...
    `core.core.Strategy.init` method,
    and set the signal vector by calling
    `core.lib.SignalStrategy.set_signal` method from within it.
...
__all__ = [
    ...
    if (
        (
            ...
            or k.isupper()  # callables from this module
        )
